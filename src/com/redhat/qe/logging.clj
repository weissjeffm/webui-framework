...
;; by Alex Taggart
;; July 27, 2009
;; modifications by Jeff Weiss, 2011
...
(ns 
  #^{:author "Alex Taggart, Timothy Pratley, Jeff Weiss",
     :doc
  ...
  something like \"fn__72$impl_write_BANG__39__auto____81\" in your logs. Java.util.logging tries to do go deeper into the stack to find the right location.
        ...
        (let [levels# {:trace java.util.logging.Level/FINEST
          ...
          (def-impl-write!
            (fn [#^java.util.logging.Logger log# level# msg# e#]
              (let [loc# (-> (Thread/currentThread) .getStackTrace
                             second
                             .getClassName (split #"\$"))]
                (if-not e#
                  (do (println (str "found loc " loc#))
                      (.logp log#
                             #^java.util.logging.Level (levels# level#)
                             #^String (first loc#)
                             #^String (second loc#)
                             #^String (str msg#)))
                  (.logp log#
